class Transaction < ActiveRecord::Base
  ...
  def self.received_by_user(user)
    Transaction.where(balance: Balance.current).where(receiver: user).or(Transaction.where(balance: Balance.current).where(receiver: User.where(name: 'Kabisa'))).order('created_at desc').page.per(20)
  end
