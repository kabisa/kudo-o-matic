...
class PostAdder
  ...
  def self.create_from_slack_command(params, team)
    ...
    raise SlackConnectionError, "Receiver is *not* connected to the ₭udo-o-Matic" if receiver_membership.nil?
    receiver = receiver_membership.user || nil
    sender_membership = team.memberships.find_by_slack_id(sender_slack_id)
    raise SlackConnectionError, "You are *not* connected to the ₭udo-o-Matic" if sender_membership.nil?
  ...
  def self.save(post, current_team)
    post.save!

    SlackService.instance.send_new_post(post, current_team)
    FcmService.instance.send_new_post(post)
