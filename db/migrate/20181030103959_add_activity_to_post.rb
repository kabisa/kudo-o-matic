...
class AddActivityToPost < ActiveRecord::Migration[5.2]
  def self.up
    ...
    execute "UPDATE posts p SET message = a.name FROM activities a WHERE a.id = p.activity_id;"
    remove_column :activities, :name
  end

  def self.down
    add_column :activities, :name, :string
    execute "UPDATE activities a SET name = p.message FROM posts p WHERE p.activity_id = a.id;"
